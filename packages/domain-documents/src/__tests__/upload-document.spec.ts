...
import { uploadDocumentCommand } from "../document-upload-command";
...
describe.concurrent("Upload Document", () => {
  ...
  it("Upload Document Command", async () => {
    const { documentId } = await uploadDocumentCommand.handler(
    ...
    const { aggregate } = await mockedDocumentEventStore.getExistingAggregate(
      documentId
    );
