...
import type { Middleware, MiddlewareFn, MiddlewareObj } from './middleware'
import type { FilteredContext, NarrowedContext } from './context'
import { Context } from './context'
import type { MaybeArray, NonemptyReadonlyArray, MaybePromise, Guard } from './util'
...
export class Composer<C extends Context> implements MiddlewareObj<C> {
    ...
    on<Filter extends tt.UpdateType | tt.MessageSubType>(
        filters: MaybeArray<Filter>,
        ...fns: NonemptyReadonlyArray<Middleware<NarrowedContext<C, tt.MountMap[Filter]>>>
    ): this
    ...
    command(
        ...
        ...fns: NonemptyReadonlyArray<Middleware<NarrowedContext<C, tt.MountMap['text']>>>
    ) {
     ...
     */
    action(triggers: Triggers<C>, ...fns: MatchedMiddleware<C, 'callback_query'>) {
     ...
     */
    inlineQuery(triggers: Triggers<C>, ...fns: MatchedMiddleware<C, 'inline_query'>) {
    ...
    gameQuery(
        ...fns: NonemptyReadonlyArray<
            Middleware<NarrowedContext<C, tg.Update.CallbackQueryUpdate<CallbackQuery.GameQuery>>>
        >
    ) {
    ...
    >(
        predicate: MaybeArray<string> | ((entity: tg.MessageEntity, s: string, ctx: C) => boolean),
    ...
    start(
        ...fns: NonemptyReadonlyArray<
            Middleware<NarrowedContext<C, tt.MountMap['text']> & { startPayload: string }>
        >
    ) {
     ...
     */
    help(...fns: NonemptyReadonlyArray<Middleware<NarrowedContext<C, tt.MountMap['text']>>>) {
     ...
     */
    settings(...fns: NonemptyReadonlyArray<Middleware<NarrowedContext<C, tt.MountMap['text']>>>) {
    ...
    >(routeFn: (ctx: C) => MaybePromise<keyof Handlers>, handlers: Handlers): Middleware<C> {
        return Composer.lazy<C>((ctx) =>
     ...
     */
    static on<Ctx extends Context, Filter extends tt.UpdateType | Guard<Ctx['update']>>(
        filters: MaybeArray<Filter>,
     ...
     * @deprecated Use filter utils instead. Support for Message subtype in `Composer.on` will be removed in Telegraf v5.
     */
    static on<Ctx extends Context, Filter extends tt.UpdateType | tt.MessageSubType>(
        filters: MaybeArray<Filter>,
        ...fns: NonemptyReadonlyArray<Middleware<NarrowedContext<Ctx, tt.MountMap[Filter]>>>
    ): MiddlewareFn<Ctx>
    ...
    static mention<C extends Context>(
        ...
        return Composer.entityText<C>('mention', normalizeTextArguments(mention, '@'), ...fns)
    }
    ...
    static hashtag<C extends Context>(
        ...
        return Composer.entityText<C>('hashtag', normalizeTextArguments(hashtag, '#'), ...fns)
    }
    ...
    static cashtag<C extends Context>(
        ...
        return Composer.entityText<C>('cashtag', normalizeTextArguments(cashtag, '$'), ...fns)
    }
    ...
    private static match<
        C extends Context,
        T extends 'message' | 'channel_post' | 'callback_query' | 'inline_query' | tt.MessageSubType
    >(
    ...
    static hears<C extends Context>(
        ...
        return Composer.on('text', Composer.match<C, 'text'>(normalizeTriggers(triggers), ...fns))
    }
...
function getText(msg: tg.Message | tg.CallbackQuery | undefined): string | undefined {
