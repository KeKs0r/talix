...
> {
    ...
    selective<T extends ForceReply | ReplyKeyboardMarkup>(this: Markup<T>, value = true) {
        return new Markup<T>({ ...this.reply_markup, selective: value })
    }

    placeholder<T extends ForceReply | ReplyKeyboardMarkup>(this: Markup<T>, placeholder: string) {
        return new Markup<T>({
            ...this.reply_markup,
...
export function inlineKeyboard(buttons: HideableIKBtn[][]): Markup<InlineKeyboardMarkup>
...
function buildKeyboard<B extends HideableKBtn | HideableIKBtn>(
    ...
    const wrapFn =
        options.wrap !== undefined
            ? options.wrap
            : (_btn: B, _index: number, currentRow: B[]) => currentRow.length >= options.columns
    let currentRow: B[] = []
    let index = 0
